...
class Gamestate {
public:
	Gamestate(const entity& p, GLuint textures, GLuint fonts, GLuint death, Mix_Chunk* sounds[]): texture(textures), font(fonts), death(death), player(p){
		for (int i = 0; i < 4; ++i) {
...
void processInputs(Gamemode& mode, Gamestate& state, Gamemenu& menu, Gamemenu& end, SDL_Event& event, bool& done,
	Mix_Music* menuMusic, Mix_Music* gameMusic) {
	...
	case MODE_MENU:
		...
		if (event.type == SDL_MOUSEBUTTONUP) {
			...
			int selection = menu.checkCollisions(inputx, inputy);
			menu.select(mode, done, gameMusic);
	...
	case MODE_END:
		...
		if (event.type == SDL_MOUSEBUTTONUP) {
			...
			float inputy = (((float)800.0f - event.button.y) / 800.0f*16.0f) - 8.0f;
			int selection = end.checkCollisions(inputx, inputy);
			end.select(mode, done, gameMusic);
			state.resetGamestate();
		}
...
int main(int argc, char *argv[])
{
	...
	Gamestate state(player, ships, fonts, death, soundEffects);
	...
	while (!done) {
		while (SDL_PollEvent(&event)) {
			...
			//inputs
			processInputs(mode, state, menu, end, event, done, music2, music1);
		}
		//update
	...
	Mix_FreeChunk(enemyShot);
	Mix_FreeChunk(enemyDoubleShot);
